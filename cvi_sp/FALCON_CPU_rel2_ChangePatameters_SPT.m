...
%% ****************************************************************************************************
function [Results,avgImg,back_est]= FALCON_CPU_rel2_ChangePatameters_SPT(filename,numFrame,dummyFrame,ADU,base,EM,Gsigma1,Gsigma2,Gsigma_ratio,speed,debug,control_dfactor,up_decon,up_refine)
%% initialization
%{
beep off;
...
wavelet_level = 6;                                                         % for background estimation
thresh_delta = 1.1;                                                        % maximum displacement of PSF
thresh_level =  .05%0.1 %0.1%0.05;                                                       % Threshold level for initial localization

% sparsity level (if too many false positive, set para = 2.5 or 3) 
para = 1.5%2%3%1.0%2%3%5%7%3% 6%5%3%2%4%3%4  %(use 3) 
disp(['para is 6, not 3 !!!!!!!!!!!!!!!!!!!!!!!!!!!!'])
%para = 100
%para = 1
range=2
...
for frame_count = 1: ceil(numFrame/frame_subset_length)
    ...
    for zz = sframe:eframe
        ...
        if ~isscalar(base)
            baseline(:,:,zz-sframe+1) = single(base(:,:,zz));
        else
            baseline = base; 
        end
    end
